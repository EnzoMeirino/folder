...
const Frame2 = () => {
  ...
  const [tdsValue, setTdsValue] = useState('N/A');
  const [energyUsage, setEnergyUsage] = useState('N/A');
  const [temperatureComment, setTemperatureComment] = useState('');
  const [phComment, setphComment] = useState('');


  useEffect(() => {
    connectToMqtt(); // Conecte-se ao servidor MQTT ao carregar o componente

    // Configurar funções de atualização dos estados com os dados MQTT recebidos
    client.onMessageArrived = (message) => {
      ...
      client.on('message', (topic, payload, packet) => {
        ...
        console.log('Nova mensagem' + payload.toString)
      })

      // Atualize os estados com base no tópico recebido
      if (topic === 'temperatura') {
        setTemperature(payload);

        if (payload > 28) {
          setTemperatureComment('A temperatura está alta hoje');
        } else if (payload < 24) {
          setTemperatureComment('A temperatura está baixa hoje');
        } else {
          setTemperatureComment('A temperatura está adequada hoje');
        }
      } else if (topic === 'ph') {
        setPhLevel(payload);
        if (payload > 7) {
          setphComment('O pH da água está alcalino');
        } else if (payload < 7) {
          setphComment('O pH da água está ácido');
        } else {
          setphComment('O pH da água está adequado hoje');
        }
      } else if (topic === 'tds') {
        setTdsValue(payload);
        ...
        <View style={[styles.modalWrapper, styles.homePosition]}>
          ...
          >
            ...
            <View style={[styles.vectorParent, styles.parentLayout]}>
              ...
              />
              <Text style={[styles.text, styles.textLayout]}>{tdsValue}</Text>
            ...
            <View style={[styles.rectangleParent, styles.parentLayout]}>
              ...
              <Text style={[styles.text1, styles.textLayout]}>{phLevel}</Text>
            </View>
            <View style={[styles.vectorGroup, styles.vectorPosition]}>
              ...
              />
              <Text style={styles.kwh}>{energyUsage} kWh</Text>
              <Text style={[styles.oOxignioEst, styles.oPhDaLayout]}>
